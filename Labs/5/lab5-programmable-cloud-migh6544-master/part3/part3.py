...
def create_instance(compute, project, zone, name):
    ...
    part_1 = open(os.path.join(os.path.dirname(__file__), 'part1.py'), 'r').read()
    service_credentials = open(os.path.join(os.path.dirname(__file__), 'service-credentials.json'), 'r').read()
    vm2_startup_script = open(os.path.join(os.path.dirname(__file__), 'vm2-startup-script.sh'), 'r').read()
    image_url = "http://storage.googleapis.com/gce-demo-input/photo.jpg"
    image_caption = "Ready for dessert?"
    config = {'name': name, 'machineType': machine_type,
        ...
        'metadata': {'items': [{'key': 'startup-script', 'value': startup_script}, { 'key' : 'part1', 'value' : part_1 }, {'key': 'service-credentials', 'value': service_credentials}, { 'key' : 'vm2-startup-script', 'value' : vm2_startup_script}]}
    }
...
def create_firewall_rule(project):
    target = "allow-5000"
    firewall_body = {"name": target, "sourceRanges": ["0.0.0.0/0"], "targetTags": [target], "allowed": [{"IPProtocol": "tcp", "ports": ["5000"]}]}
    try:
...
def add_firewall_rule_to_compute_node(project, zone, instance, target_tag, fingerprint):
    tags_body = {"items": [target_tag], "fingerprint": fingerprint}
    try:
...
if __name__ == '__main__':
    ...
    Instance_Name = 'csci4253-lab5-part3'
    Zone = 'us-west1-b'  

    operation = create_instance(service, Project_ID, Zone, Instance_Name)
